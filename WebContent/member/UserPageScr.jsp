...
<%
	...
	ArrayList<ProBoardDto> sc_boa_list = null;
	MemberDto UserInfo=(MemberDto)request.getAttribute("useinfo");
	if(request.getAttribute("sc_boa_list")!=null){
		
		sc_boa_list = (ArrayList<ProBoardDto>)request.getAttribute("sc_boa_list");
	}
...
<main> <!--스크랩, 지원, 등록 정보 시작-->
    <div role="tab">
        ...
        <ul class="tabs">
          ...
          <%if(logincheck==1){
          		if(MemberDto.getMem_no()==UserInfo.getMem_no()){%>
        	 <li class="tab active"><a href="#Support" class="lable">스크랩</a></li> 
          <%	 } 
          	}%>
                  ...
                  <div class="col-sm-12"><!--지원카드 시작-->
                      ...
                      <div id="dede">
                          <div id="card_User">
                            ...
                            <p class="projact_card_U_font"><%=sc_boa_list.get(i).getMem_nickname()%><br><small><%=sc_boa_list.get(i).getMem_job()%></small></p>
                          </div>
                        
                          <div id="card_content">
                            <div class="oneline">
                              <ul>
                                <li class="oneline_team"><%=sc_boa_list.get(i).getBoa_region()%></li>
                                <li><a href="./ProDetail.bo?no=<%=sc_boa_list.get(i).getBoa_no()%>"><%=sc_boa_list.get(i).getBoa_title()%></a>  
                                  <button class='star' type="button" title="스크랩" data-toggle="tooltip" onclick="star(<%=sc_boa_list.get(i).getBoa_no()%>)" title="스크랩" data-original-title="Default tooltip">
                                 <%
                                	 for(int j=0; j<sc_list.size(); j++){
                                			if(sc_list.get(j)==sc_boa_list.get(i).getBoa_no()){%>
                                			<img id="i_<%=sc_boa_list.get(i).getBoa_no()%>" src="<%=request.getContextPath()%>/image/yellowstar.png">
                                	 		<% roop=1;}
                                 	}if(roop!=1){%>
                                 		<img id="i_<%=sc_boa_list.get(i).getBoa_no()%>" src="<%=request.getContextPath()%>/image/graystar.png">
                                 	<%}roop=0; %>
                                  
                                              
                                </li>
                                <li><small style="float:right;">등록날짜|&nbsp;&nbsp;<%=sc_boa_list.get(i).getBoa_reg_date()%></small></li>
                              </ul> 
                            </div>
                            <div class="twoline">
                              <table class="table">
                                <tr>
                                  ...
                                  <td ><b id="twoline_content"><%=sc_boa_list.get(i).getBoa_job()%></b></td>
                                </tr>
                                <tr>
                                  <td id="twoline_title">작업기간</td>
                                  <td><b id="twoline_content">
                                   <%if(sc_boa_list.get(i).getBoa_pro_period().equals("Period_Week")){%>
                        			~1주일
                        			<%}else if(sc_boa_list.get(i).getBoa_pro_period().equals("Period_One")){ %>
                        			1주일 ~ 1개월
                        			<%}else if(sc_boa_list.get(i).getBoa_pro_period().equals("Period_Three")){ %>
                        			1개월 ~ 3개월
                        			<%}else if(sc_boa_list.get(i).getBoa_pro_period().equals("Period_Six")){ %>
                        			3개월 ~ 6개월
                        			<%}else if(sc_boa_list.get(i).getBoa_pro_period().equals("Period_Year")){ %>
                        			6개월 ~ 1년
                        			<%} %>
                                  </b></td>
                                </tr>
                                <tr>
                                  ...
                                  <td><b id="twoline_content">
                                  <%if(sc_boa_list.get(i).getBoa_d_day()>0){ %>
                    				  D - <%=sc_boa_list.get(i).getBoa_d_day()%>
                      				<%}else if(sc_boa_list.get(i).getBoa_d_day()==0){ %>
                      				TODAY
                      				<%}else if(sc_boa_list.get(i).getBoa_d_day()<0){  int d_day=Math.abs(sc_boa_list.get(i).getBoa_d_day()); %>
                      				D + <%=d_day%>
                      				<%} %>
                                  </b></td>
                                </tr>
                                <tr>
                                  ...
                                  <td><b id="twoline_content"><%=sc_boa_list.get(i).getApp_number()%>/<%=sc_boa_list.get(i).getBoa_num()%> &nbsp;명</b></td>
                                </tr>
                              </table>
                            </div>
                            <div class="threeline">
                                ...
                                <ul>
                                 <%for(int j=0; j<sc_boa_list.get(i).getBoa_hashtag().size(); j++ ){%>
                                  <li><a href="./Project.bo?TagSearch=<%=sc_boa_list.get(i).getBoa_hashtag().get(j)%>">
                                  #<%=sc_boa_list.get(i).getBoa_hashtag().get(j)%>
                                  </a></li>
                                  <%}%>
                                </ul>
                      ...
                      <nav id="my_next"> <!--페이지 네이션-->
                            <ul class="pagination">
                              ...
                              <%for(int i=0; i< total; i++){%>
                            	 <li id="page_<%=i+1%>"><a href="./UserPageScr.do?mem_no=<%=UserInfo.getMem_no()%>&page=<%=i+1%>" ><%=i+1%></a></li> 
                              <% } %>
