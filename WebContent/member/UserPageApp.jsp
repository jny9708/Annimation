...
<%
	...
	ArrayList<ProBoardDto> app_parent_list = null;
	MemberDto UserInfo=(MemberDto)request.getAttribute("useinfo");
	if(request.getAttribute("app_parent_list")!=null){
		app_parent_list = (ArrayList<ProBoardDto>) request.getAttribute("app_parent_list");
	}
  ...
  <div class="wrap">
    <div class="side front">
     ...
     <% if(logincheck==1){
        	...
        	<button class="btn my_modify" type="button" name="sp_deletion" onclick="location.href='./UserUpdateForm.do'">회원 수정 및 탈퇴</button>  
          <%} 
          }%>
  </div>
...
<main> <!--스크랩, 지원, 등록 정보 시작-->
    <div role="tab">
        <!-- 해더 tabs -->
        <ul class="tabs">
          <li class="tab active"><a href="#Support" class="lable" >지원글</a></li>
          <li class="tab"><a href="./UserPageWri.do?mem_no=<%=UserInfo.getMem_no()%>" class="lable" >등록글</a></li>
          <%if(logincheck==1){  
          		if(MemberDto.getMem_no()==UserInfo.getMem_no()){%>
          ...
          <div role="tabpanel" class="tab-pane active" id="Support">
              <div class="row">
                  <div class="col-sm-12"><!--지원카드 시작-->
                      ...
                      <div id="dede">
                          <div id="card_User">
                            <a href="./UserPageApp.do?mem_no=<%=app_parent_list.get(i).getMem_no()%>">
                            <%if(app_parent_list.get(i).getMem_icon().equals("profile.jpg")){%>
                            	<img src="<%=request.getContextPath()%>/image/profile.jpg" alt="User-img" class="projact_card_U img-circle" data-toggle="tooltip" title="<%=app_parent_list.get(i).getMem_nickname()%> 페이지 보기" data-original-title="Default tooltip">	
                            <%}else{
                            	System.out.println("아이콘경로있음");%>
                				<img src="<%=request.getContextPath()%>/usericon/<%=app_parent_list.get(i).getMem_icon()%>" alt="User-img" class="projact_card_U img-circle" data-toggle="tooltip" title="<%=app_parent_list.get(i).getMem_nickname()%> 페이지 보기" data-original-title="Default tooltip">
                            <% } %>
                            
                            </a>
                            <p class="projact_card_U_font"><%=app_parent_list.get(i).getMem_nickname()%><br><small><%=app_parent_list.get(i).getMem_job()%></small></p>
                          </div>
                        
                          <div id="card_content">
                            <div class="oneline">
                              <ul>
                                <li class="oneline_team"><%=app_parent_list.get(i).getBoa_region()%></li>
                                <li><a href="./ProDetail.bo?no=<%=app_parent_list.get(i).getBoa_no()%>"><%=app_parent_list.get(i).getBoa_title()%></a>  
                                  <button class='star' type="button" title="스크랩" data-toggle="tooltip" onclick="star(<%=app_parent_list.get(i).getBoa_no()%>)" title="스크랩" data-original-title="Default tooltip">
                                 <%
                                	 for(int j=0; j<sc_list.size(); j++){
                                			if(sc_list.get(j)==app_parent_list.get(i).getBoa_no()){%>
                                			<img id="i_<%=app_parent_list.get(i).getBoa_no()%>" src="<%=request.getContextPath()%>/image/yellowstar.png">
                                	 		<% roop=1;}
                                 	}if(roop!=1){%>
                                 		<img id="i_<%=app_parent_list.get(i).getBoa_no()%>" src="<%=request.getContextPath()%>/image/graystar.png">
                                 	<%}roop=0; %>
                                  
                                              
                                </li>
                                <li><small style="float:right;">등록날짜|&nbsp;&nbsp;<%=app_parent_list.get(i).getBoa_reg_date()%></small></li>
                              </ul> 
                            </div>
                            <div class="twoline">
                              <table class="table">
                                <tr>
                                  ...
                                  <td ><b id="twoline_content"><%=app_parent_list.get(i).getBoa_job()%></b></td>
                                </tr>
                                <tr>
                                  <td id="twoline_title">작업기간</td>
                                  <td><b id="twoline_content">
                                   <%if(app_parent_list.get(i).getBoa_pro_period().equals("Period_Week")){%>
                        			~1주일
                        			<%}else if(app_parent_list.get(i).getBoa_pro_period().equals("Period_One")){ %>
                        			1주일 ~ 1개월
                        			<%}else if(app_parent_list.get(i).getBoa_pro_period().equals("Period_Three")){ %>
                        			1개월 ~ 3개월
                        			<%}else if(app_parent_list.get(i).getBoa_pro_period().equals("Period_Six")){ %>
                        			3개월 ~ 6개월
                        			<%}else if(app_parent_list.get(i).getBoa_pro_period().equals("Period_Year")){ %>
                        			6개월 ~ 1년
                        			<%} %>
                                  </b></td>
                                </tr>
                                <tr>
                                  ...
                                  <td><b id="twoline_content">
                                  <%if(app_parent_list.get(i).getBoa_d_day()>0){ %>
                    				  D - <%=app_parent_list.get(i).getBoa_d_day()%>
                      				<%}else if(app_parent_list.get(i).getBoa_d_day()==0){ %>
                      				TODAY
                      				<%}else if(app_parent_list.get(i).getBoa_d_day()<0){  int d_day=Math.abs(app_parent_list.get(i).getBoa_d_day()); %>
                      				D + <%=d_day%>
                      				<%} %>
                                  </b></td>
                                </tr>
                                <tr>
                                  ...
                                  <td><b id="twoline_content"><%=app_parent_list.get(i).getApp_number()%>/<%=app_parent_list.get(i).getBoa_num()%> &nbsp;명</b></td>
                                </tr>
                              </table>
                            </div>
                            <div class="threeline">
                                ...
                                <ul>
                                 <%for(int j=0; j<app_parent_list.get(i).getBoa_hashtag().size(); j++ ){%>
                                  <li><a href="./Project.bo?TagSearch=<%=app_parent_list.get(i).getBoa_hashtag().get(j)%>">
                                  #<%=app_parent_list.get(i).getBoa_hashtag().get(j)%>
                                  </a></li>
                                  <%}%>
                                </ul>
